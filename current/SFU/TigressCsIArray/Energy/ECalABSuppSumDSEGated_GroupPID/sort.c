...
int main(int argc, char *argv[])
{
  ...
  if((argc!=7)&&(argc!=8))
    {
      printf("TigressCsIArray_ECalABSuppSumDSEGated_GroupPID master_file_name supLow supHigh useCharge gateELow gateEHigh fudge_factor\n");
      ...
      printf("Sorted spectrum will be mapped by group rather than Tigress ring.\n");
  ...
  printf("Program attempts to generate energy gated gamma ray spectra where the energy gate is applied to Doppler unshifted data, but the gated spectrum written to disk is NOT Doppler unshifted.\n");
  ...
  if(name->flag.cluster_file==1)
    {
      printf("Sorting ECalABSuppGroupSum histograms for TIGRESS clovers and cores based upon the cluster file: %s\n",name->fname.cluster_file);
  ...
  if(name->flag.root_gate_file==1)
    {
      ...
      TFile* gateFile=new TFile(name->fname.root_gate_file,"READ");
    }
  else
    {
  ...
  sprintf(FileName,"ECalABSuppGroupSumDS_c%.0f_w%.0fgated.mca",avgGateE,gateWidth);
  ...
  sprintf(FileName,"ECalABSuppGroupSumDS_c%.0f_w%.0fgate.mca",avgGateE,gateWidth);
  if((output=fopen(FileName,"w"))==NULL)
    {
      printf("ERROR!!! I cannot open the mca file!\n");
      exit(EXIT_FAILURE);
  ...
  sprintf(FileName,"ECalABSuppGroupSumDS_c%.0f_w%.0fproj.mca",avgGateE,gateWidth);
  if((output=fopen(FileName,"w"))==NULL)
    {
      printf("ERROR!!! I cannot open the mca file!\n");
      exit(EXIT_FAILURE);
