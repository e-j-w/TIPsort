          ...
          for(pos2=1;pos2<NCSI;pos2++) //look at each CsI position
            if(pos2!=pos1)
              if((cev->csiarray.h.THP[pos2/64]&(one<<pos2%64))!=0) //is there a hit in the detector?
                {
                  thit=cev->csiarray.csi[pos2].T/cal_par->csiarray.contr_t;
...
int main(int argc, char *argv[])
{
  ...
  if(argc!=2)
    {
      printf("\n ./separate_CsIArray_TTCalDiffDropOutliers master_file_name\n");
      printf("\n Separates out particle-particle coincidences where the particles arrive within a (calibrated) time gate.  Discards any particle hits which arrive outside of the time gate (with respect to all other hits).\nThe time gate length is specified in the CsI array calibration parameters file (under 'CSIARRAY_TTCal_gate_length').\n");
      exit(-1);
    }
